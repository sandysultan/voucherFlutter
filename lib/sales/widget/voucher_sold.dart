  ...
  VoucherSold({
    ...
    required ValueChanged<VoucherRecap?> onChanged,
          ...
          builder: (FormFieldState<VoucherRecap> field) {
            return _VoucherTableView(voucherRecap: field.value,onChanged:(voucherRecap){
  ...
  Widget build(BuildContext context) {
    ...
    var list = [
      const TableRow(children: [
        Padding(
          ...
          child: Text('Voucher',textAlign: TextAlign.center,),
        ),
        Padding(
          padding: EdgeInsets.symmetric(vertical: 8),
        ...
        Padding(
          padding: EdgeInsets.symmetric(vertical: 8),
          child: Text('Restock',textAlign: TextAlign.center,),
        ),
      ]),
    ];
    list.addAll(widget.voucherRecap?.vouchers
        ...
        setState(() {
          ...
          // logger.d('_subTotal ' + _subTotal.toString());
          widget.onChanged(VoucherRecap(vouchers: _vouchers??[], subTotal: _subTotal));
        });
      }),
    )
        .toList() ??
      ...
      children: [
        ...
        Padding(
          ...
          child: Table(
            ...
            children: _vouchers?.map((e) =>
                TableRow(
                  children: [
                    Container(),
                    Text(e.name),
                    Text(formatter.format(e.price),textAlign: TextAlign.right,),
                    const Text(' x '),
                    Text(formatter.format(e.stock-e.balance),textAlign: TextAlign.right,),
                    const Text(' = '),
                    Text(formatter.format(e.price * (e.stock-e.balance)),textAlign: TextAlign.right,),
                  ],
                ))
                .toList()??[],
          ),
        ...
        Padding(
          ...
          child: Table(
            ...
            children: [
              TableRow(children: [const Text('Subtotal',textAlign: TextAlign.end,),
                const Text(' = '),
                Text(formatter.format(_subTotal),textAlign: TextAlign.right,)
              ])
              ,],
          ),
        ),
...
class _NumberField extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    controller.selection= TextSelection(baseOffset: s.length, extentOffset: s.length);
    return
      Padding(
...
class VoucherItem {
  ...
  VoucherItem(
      this.id, this.name, this.price, this.stock, this.balance, this.restock);
}
